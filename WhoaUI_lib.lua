...
local function loadNamedConfig(name)
    ...
    if ok then _applyData(d) end; return ok
end
...
local function addTab(name)
    ...
    local function makeCol(xScale,xOffset,wOffset)
        local col=new("ScrollingFrame",{Position=UDim2.new(xScale,xOffset,0,6),Size=UDim2.new(0.5,wOffset,1,-12),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=3,ScrollBarImageColor3=T.BD,CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y},frame)
...
local function makeSection(parent,title)
    ...
    function api:AddCheckbox(cfg)
        local flg=cfg.Flag or cfg.Name; local on=Flags[flg]~=nil and Flags[flg] or (cfg.Default==true); Flags[flg]=on
        ...
        if cfg.Keybind then local kb=new("TextLabel",{AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-38,0.5,0),Size=UDim2.new(0,28,0,16),BackgroundColor3=T.B2,Text=string.upper(string.sub(cfg.Keybind,1,3)),TextColor3=T.MT,Font=Enum.Font.GothamBold,TextSize=9},row); cr(4,kb); st(T.BD,1,kb) end
        ...
        local function upd()
            ...
            else cbFill.Visible=false; tick.Visible=false; tw(cbBG,{Size=UDim2.new(0,22,0,22)}); tw(cbSt,{Color=T.BD,Thickness=1.5}) end; saveConfig()
        end
        table.insert(AL,function(c) if on then cbFill.BackgroundColor3=c; cbSt.Color=c end end)
        if cfg.Keybind then local kc=Enum.KeyCode[cfg.Keybind]; if kc then UIS.InputBegan:Connect(function(i,gpe) if gpe then return end; if i.KeyCode==kc then on=not on; Flags[flg]=on; upd(); if cfg.Callback then cfg.Callback(on) end end end) end end
        ...
        btn.MouseButton1Click:Connect(function() on=not on; Flags[flg]=on; upd(); if cfg.Callback then cfg.Callback(on) end end); upd()
    end

    function api:AddSlider(cfg)
        local flg=cfg.Flag or cfg.Name; local val=(Flags[flg]~=nil) and Flags[flg] or (cfg.Default or cfg.Min or 0); local dec=cfg.Decimals or 0; Flags[flg]=val
        table.insert(searchItems,{label=cfg.Name,keywords=cfg.Name..(cfg.Flag or ""),tab=tabName,switch=tabSwitch})
        ...
        local grad=new("UIGradient",{Color=ColorSequence.new{ColorSequenceKeypoint.new(0,T.A),ColorSequenceKeypoint.new(1,Color3.fromRGB(255,180,220))}},fil); table.insert(AL,function(c) fil.BackgroundColor3=c; grad.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,c),ColorSequenceKeypoint.new(1,Color3.fromRGB(255,180,220))} end)
        ...
        local function setVal(v)
            v=math.clamp(v,cfg.Min,cfg.Max); v=dec==0 and math.round(v) or math.floor(v*10^dec+0.5)/10^dec; val=v; Flags[flg]=v; local p=(v-cfg.Min)/(cfg.Max-cfg.Min)
            tw(fil,{Size=UDim2.new(p,0,1,0)},0.04); tw(thumb,{Position=UDim2.new(p,0,0.5,0)},0.04); vl.Text=fmt(v).." / "..fmt(cfg.Max); if cfg.Callback then cfg.Callback(v) end; saveConfig()
        end
    ...
    function api:AddDropdown(cfg)
        local flg=cfg.Flag or cfg.Name; local defaultVal=Flags[flg] or cfg.Default or (cfg.Items and cfg.Items[1]) or ""; Flags[flg]=defaultVal
        table.insert(searchItems,{label=cfg.Name,keywords=cfg.Name..(cfg.Flag or ""),tab=tabName,switch=tabSwitch})
        ...
        sb.MouseButton1Click:Connect(function()
            ...
            if open then
                ...
                task.delay(0.05,function() if not open then return end; outsideConn=UIS.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then closeDropdown() end end) end)
    ...
    function api:AddTextBox(cfg)
        ...
        tb.Focused:Connect(function() tw(tSt,{Color=T.A}) end); tb.FocusLost:Connect(function(e) tw(tSt,{Color=T.BD}); Flags[flg]=tb.Text; if e and cfg.Callback then cfg.Callback(tb.Text) end; saveConfig() end)
        return {Get=function() return tb.Text end}
    end

    function api:AddColorPicker(cfg)
        local flg=cfg.Flag or cfg.Name; local savedColor=Flags[flg]; local cur=(typeof(savedColor)=="Color3" and savedColor) or cfg.Default or T.A; Flags[flg]=cur
        table.insert(searchItems,{label=cfg.Name,keywords=cfg.Name..(cfg.Flag or ""),tab=tabName,switch=tabSwitch})
        ...
        local satGrad=new("Frame",{Size=UDim2.new(1,0,1,0),BackgroundColor3=Color3.new(1,1,1)},svBox); new("UIGradient",{Transparency=NumberSequence.new{NumberSequenceKeypoint.new(0,0),NumberSequenceKeypoint.new(1,1)}},satGrad)
        local valGrad=new("Frame",{Size=UDim2.new(1,0,1,0),BackgroundColor3=Color3.new(0,0,0)},svBox); new("UIGradient",{Transparency=NumberSequence.new{NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)},Rotation=90},valGrad)
        ...
        hexBox.FocusLost:Connect(function() tw(hexSt,{Color=T.BD}); local c=hexToColor(hexBox.Text); if c then cur=c; cH,cS,cV=Color3.toHSV(c); sw.BackgroundColor3=c; svBox.BackgroundColor3=Color3.fromHSV(cH,1,1); svThumb.Position=UDim2.new(cS,0,1-cV,0); hThumb.Position=UDim2.new(cH,0,0.5,0); Flags[flg]=c; if cfg.Callback then cfg.Callback(c) end; saveConfig() end; hexBox.Text=colorToHex(cur) end)
    ...
    function api:AddKeybind(cfg)
        ...
        badge.MouseButton1Click:Connect(function()
            ...
            local conn; conn=UIS.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.Keyboard then listening=false; listeningForKey=false; cur=i.KeyCode; Flags[flg]=cur; if flg=="tkey" then toggleKey=cur end; badge.Text=cur.Name; badge.TextColor3=T.A; tw(badge,{BackgroundColor3=T.B2}); tw(bSt,{Color=T.BD}); if cfg.Callback then cfg.Callback(cur) end; saveConfig(); conn:Disconnect() end end)
        end)
    end
...
local function Setup(cfg)
    ...
    if cfg.Version then SCRIPT_VERSION=cfg.Version; wmVLabel.Text=cfg.Version end
...
return {
    ...
    StartSnow=startSnow, StopSnow=stopSnow, Flags=Flags, pbNameLabel=pbNameLabel,
    avImg=avImg, wmFrame=wmFrame, wmNameLabel=wmNameLabel, realAvatar=function() return realAvatar end,
