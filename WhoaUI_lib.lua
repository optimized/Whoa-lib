...
local AL = {}
local function setA(c)
    T.A = c
    for _, fn in ipairs(AL) do pcall(fn, c) end
end
...
local function _applyData(data)
...
local function _addToCfgList(name)
    ...
    pcall(function() writefile(CFG_INDEX,HS:JSONEncode(list)) end)
end
local function saveNamedConfig(name)
    if not writefile or not name or name=="" then return end
    pcall(function() writefile(CFG_PREFIX..name..".json",HS:JSONEncode(_encodeFlags())) end)
    _addToCfgList(name)
end
local function loadNamedConfig(name)
    ...
    if ok then _applyData(d) end
    return ok
end
local function getAutoLoad()
    if not readfile or not isfile or not isfile(CFG_AUTO) then return nil end
    local n=readfile(CFG_AUTO):gsub("%s","")
    return n~="" and n or nil
end
local function setAutoLoad(name)
    pcall(function() if writefile then writefile(CFG_AUTO, name or "") end end)
end
...
loadConfig()
local _autoName = getAutoLoad()
if _autoName then loadNamedConfig(_autoName) end

-- ── HELPERS ───────────────────────────────────────────────
local function tw(o, p, t, s)
    TS:Create(o, TweenInfo.new(t or 0.15, s or Enum.EasingStyle.Quad, Enum.EasingDirection.Out), p):Play()
end
...
local function tl(props, parent)
    ...
    s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = o
    return o
end
...
local SG = new("ScreenGui", {
    Name="WhoaUI", ResetOnSpawn=false,
    ZIndexBehavior=Enum.ZIndexBehavior.Sibling,
    IgnoreGuiInset=true, DisplayOrder=99,
})
pcall(function()
...
if KEY_ENABLED then
    ...
    if not unlocked then
        ...
        local md  = new("Frame",{AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0.5,0,0.5,0),Size=UDim2.new(0,360,0,270),BackgroundColor3=T.B3,ZIndex=201},ov)
        cr(12,md); st(T.BD,1,md)
        ...
        local ib  = new("Frame",{Position=UDim2.new(0,20,0,96),Size=UDim2.new(1,-40,0,36),BackgroundColor3=T.B4,ZIndex=202},md); cr(7,ib)
        ...
        ki.Focused:Connect(function() tw(iSt,{Color=T.A}) end)
        ki.FocusLost:Connect(function() tw(iSt,{Color=T.BD}) end)
        ...
        local function tryKey()
            ...
            if valid then
                ...
                el.Text="Incorrect key. Please try again."
                tw(iSt,{Color=Color3.fromRGB(200,60,60)},0.1)
...
local nColors = {Success=Color3.fromRGB(60,220,130),Error=Color3.fromRGB(255,75,75),Warning=Color3.fromRGB(255,160,40)}

local function Notify(title, body, ntype, dur)
    local col = nColors[ntype] or T.A
...
local Win = new("Frame",{
    Name="Window", AnchorPoint=Vector2.new(0.5,0),
    Position=UDim2.new(0.5,0,0.5,-WH/2),
    Size=UDim2.new(0,WW,0,WH),
    BackgroundColor3=T.B1, ClipsDescendants=true, ZIndex=10,
},SG)
cr(12,Win)
local winSt = st(T.A2, 1.5, Win)
...
local pb = new("Frame",{AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-42,0.5,0),Size=UDim2.new(0,28,0,28),AutomaticSize=Enum.AutomaticSize.X,BackgroundColor3=T.B4},TBar); cr(6,pb); st(T.BD,1,pb)
pad(0,0,3,10,pb)
...
task.spawn(function()
    pcall(function()
        realAvatar = Players:GetUserThumbnailAsync(LP.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size60x60)
        avImg.Image = realAvatar
    end)
end)
...
mb.MouseButton1Click:Connect(function()
    winMin = not winMin
    tw(Win,{Size=winMin and UDim2.new(0,WW,0,54) or UDim2.new(0,WW,0,WH)},0.22)
end)
...
local function setVisible(v)
    winOpen=v; Win.Visible=v; bgOverlay.Visible=v; snowCont.Visible=v
end
...
local searchBg = new("Frame",{AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-8,0.5,0),Size=UDim2.new(0,108,0,24),BackgroundColor3=T.B2},TopRow)
cr(6,searchBg); local searchSt=st(T.BD,1,searchBg)
...
local searchOverlay = new("ScrollingFrame",{
    Size=UDim2.new(1,0,1,0),BackgroundColor3=T.B1,BackgroundTransparency=0,
    BorderSizePixel=0,ScrollBarThickness=3,ScrollBarImageColor3=T.BD,
...
local function rebuildSearch(query)
    ...
    if query == "" then searchOverlay.Visible = false; return end
    ...
    for _, item in ipairs(searchItems) do
        local kw = item.keywords:lower():gsub("%s","")
        if kw:find(query, 1, true) then
            found = found + 1
            ...
            rowBtn.MouseLeave:Connect(function() tw(res,{BackgroundColor3=T.B3},0.08) end)
            local function doNav()
                if item.switch then item.switch() end
                searchBox.Text = ""
            end
            rowBtn.MouseButton1Click:Connect(doNav)
            goBtn.MouseButton1Click:Connect(doNav)
        end
    end
    if found == 0 then
        tl({Size=UDim2.new(1,0,0,36),BackgroundTransparency=1,Text="No results found",TextColor3=T.MT,Font=Enum.Font.FredokaOne,TextSize=14,ZIndex=51},searchOverlay)
    end
end
...
local function scaleWindow()
    ...
    if s>=1 then Win.Size=UDim2.new(0,WW,0,WH)
    else s=s*0.93; Win.Size=UDim2.new(0,math.floor(WW*s),0,math.floor(WH*s)) end
...
local function addTab(name)
    ...
    local function makeCol(xScale,xOffset,wOffset)
        local col=new("ScrollingFrame",{
            Position=UDim2.new(xScale,xOffset,0,6),Size=UDim2.new(0.5,wOffset,1,-12),
            BackgroundTransparency=1,BorderSizePixel=0,
            ScrollBarThickness=3,ScrollBarImageColor3=T.BD,
            CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y,
        },frame)
...
local function makeSection(parent, title)
    ...
    local lo=0; local function nl() lo=lo+1; return lo end
    ...
    UIS.InputChanged:Connect(function(i)
        if slideActive and i.UserInputType==Enum.UserInputType.MouseMovement then
            local cb=slideCallbacks[slideActive]; if cb then cb(i.Position.X,i.Position.Y) end
        end
    end)
    UIS.InputEnded:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then slideActive=nil end
    end)
    local function regSlide(id,startFn,moveFn,obj)
        slideCallbacks[id]=moveFn
        obj.InputBegan:Connect(function(i)
            if i.UserInputType==Enum.UserInputType.MouseButton1 then slideActive=id; startFn(i.Position.X,i.Position.Y) end
        end)
    end

    function api:AddCheckbox(cfg)
        local flg=cfg.Flag or cfg.Name
        local on=Flags[flg]~=nil and Flags[flg] or (cfg.Default==true)
        Flags[flg]=on
        ...
        if cfg.Keybind then
            local kb=new("TextLabel",{AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-38,0.5,0),Size=UDim2.new(0,28,0,16),BackgroundColor3=T.B2,Text=string.upper(string.sub(cfg.Keybind,1,3)),TextColor3=T.MT,Font=Enum.Font.GothamBold,TextSize=9},row); cr(4,kb); st(T.BD,1,kb)
        end
        local cbBG=new("Frame",{AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-10,0.5,0),Size=UDim2.new(0,22,0,22),BackgroundColor3=T.B1},row)
        cr(6,cbBG); local cbSt=st(T.BD,1.5,cbBG)
        ...
        if cfg.Keybind then
            ...
            if kc then UIS.InputBegan:Connect(function(i,gpe) if gpe then return end; if i.KeyCode==kc then on=not on; Flags[flg]=on; upd(); if cfg.Callback then cfg.Callback(on) end end end) end
        end
    ...
    function api:AddSlider(cfg)
        ...
        local vl=new("TextButton",{AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,-10,0,6),Size=UDim2.new(0,94,0,20),BackgroundColor3=T.B2,Text=fmt(val).." / "..fmt(cfg.Max),TextColor3=T.A,Font=Enum.Font.FredokaOne,TextSize=12},wr); cr(4,vl); st(T.BD,1,vl)
        table.insert(AL,function(c) vl.TextColor3=c end)
        local editBox=new("TextBox",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="",TextColor3=T.A,Font=Enum.Font.Gotham,TextSize=12,Visible=false,ClearTextOnFocus=true,ZIndex=2},vl)
        ...
        local function setVal(v)
            v=math.clamp(v,cfg.Min,cfg.Max); v=dec==0 and math.round(v) or math.floor(v*10^dec+0.5)/10^dec
            val=v; Flags[flg]=v; local p=(v-cfg.Min)/(cfg.Max-cfg.Min)
            tw(fil,{Size=UDim2.new(p,0,1,0)},0.04); tw(thumb,{Position=UDim2.new(p,0,0.5,0)},0.04)
            vl.Text=fmt(v).." / "..fmt(cfg.Max); if cfg.Callback then cfg.Callback(v) end; saveConfig()
        end
        vl.MouseButton1Click:Connect(function() editBox.Text=fmt(val); editBox.Visible=true; vl.Text=""; editBox:CaptureFocus() end)
        editBox.FocusLost:Connect(function() editBox.Visible=false; local n=tonumber(editBox.Text); if n then setVal(n) end; vl.Text=fmt(val).." / "..fmt(cfg.Max) end)
        local function fromX(x) setVal(cfg.Min+math.clamp((x-track.AbsolutePosition.X)/track.AbsoluteSize.X,0,1)*(cfg.Max-cfg.Min)) end
        regSlide("slider_"..flg,fromX,function(x,_) fromX(x) end,track)
        wr.MouseEnter:Connect(function() tw(hbg,{BackgroundTransparency=0.88},0.1) end)
        wr.MouseLeave:Connect(function() tw(hbg,{BackgroundTransparency=1},0.1) end)
        return {Set=setVal,Get=function() return val end}
    end

    function api:AddDropdown(cfg)
        local flg=cfg.Flag or cfg.Name
        local defaultVal=Flags[flg] or cfg.Default or (cfg.Items and cfg.Items[1]) or ""
        Flags[flg]=defaultVal
        table.insert(searchItems,{label=cfg.Name,keywords=cfg.Name..(cfg.Flag or ""),tab=tabName,switch=tabSwitch})
        ...
        local open,lf,outsideConn=false,nil,nil
        ...
        sb.MouseButton1Click:Connect(function()
            ...
            if open then
                ...
                for _,item in ipairs(cfg.Items or {}) do
                    ...
                    op.MouseEnter:Connect(function() tw(op,{BackgroundColor3=T.B4},0.08) end)
                    op.MouseLeave:Connect(function() tw(op,{BackgroundColor3=T.B2},0.08) end)
                    op.MouseButton1Click:Connect(function() Flags[flg]=item; selLbl.Text=item; closeDropdown(); if cfg.Callback then cfg.Callback(item) end; saveConfig() end)
                end
                local h=#(cfg.Items or {})*26
                tw(lf,{Size=UDim2.new(1,-20,0,h)},0.15); tw(wr,{Size=UDim2.new(1,0,0,56+h+2)},0.15)
                task.delay(0.05,function()
                    if not open then return end
                    outsideConn=UIS.InputBegan:Connect(function(i)
                        if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then closeDropdown() end
                    end)
                end)
    ...
    function api:AddButton(cfg)
        ...
        local btn=new("TextButton",{Position=UDim2.new(0,10,0.5,-13),Size=UDim2.new(1,-20,0,26),BackgroundColor3=T.B2,Text=cfg.Name,TextColor3=Color3.new(1,1,1),Font=Enum.Font.FredokaOne,TextSize=14},wr)
        cr(6,btn); local bSt=st(T.BD,1,btn)
        ...
        btn.MouseButton1Down:Connect(function() tw(btn,{BackgroundColor3=T.B0},0.07) end)
        btn.MouseButton1Click:Connect(function() tw(btn,{BackgroundColor3=T.B2},0.1); if cfg.Callback then cfg.Callback() end end)
    end

    function api:AddTextBox(cfg)
        local flg=cfg.Flag or cfg.Name
        Flags[flg]=Flags[flg] or cfg.Default or ""
        table.insert(searchItems,{label=cfg.Name,keywords=cfg.Name..(cfg.Flag or ""),tab=tabName,switch=tabSwitch})
        ...
        local bg=new("Frame",{Position=UDim2.new(0,10,0,27),Size=UDim2.new(1,-20,0,24),BackgroundColor3=T.B2},wr); cr(5,bg)
        ...
        tb.FocusLost:Connect(function(e) tw(tSt,{Color=T.BD}); Flags[flg]=tb.Text; if e and cfg.Callback then cfg.Callback(tb.Text) end; saveConfig() end)
        return {Get=function() return tb.Text end}
    end

    function api:AddColorPicker(cfg)
        local flg=cfg.Flag or cfg.Name
        local savedColor=Flags[flg]
        local cur=(typeof(savedColor)=="Color3" and savedColor) or cfg.Default or T.A
        Flags[flg]=cur
        table.insert(searchItems,{label=cfg.Name,keywords=cfg.Name..(cfg.Flag or ""),tab=tabName,switch=tabSwitch})
        local cH,cS,cV=Color3.toHSV(cur)
        local function rebuildColor() cur=Color3.fromHSV(cH,cS,cV); Flags[flg]=cur; if cfg.Callback then cfg.Callback(cur) end; saveConfig() end
        local row=new("Frame",{Size=UDim2.new(1,0,0,36),BackgroundTransparency=1,LayoutOrder=nl()},body)
        ...
        local satGrad=new("Frame",{Size=UDim2.new(1,0,1,0),BackgroundColor3=Color3.new(1,1,1)},svBox)
        new("UIGradient",{Transparency=NumberSequence.new{NumberSequenceKeypoint.new(0,0),NumberSequenceKeypoint.new(1,1)}},satGrad)
        local valGrad=new("Frame",{Size=UDim2.new(1,0,1,0),BackgroundColor3=Color3.new(0,0,0)},svBox)
        new("UIGradient",{Transparency=NumberSequence.new{NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)},Rotation=90},valGrad)
        local svThumb=new("Frame",{AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(cS,0,1-cV,0),Size=UDim2.new(0,14,0,14),BackgroundColor3=Color3.new(1,1,1),ZIndex=3},svBox); cr(99,svThumb); st(Color3.new(0,0,0),1.5,svThumb)
        local function applySV(x,y) cS=math.clamp((x-svBox.AbsolutePosition.X)/svBox.AbsoluteSize.X,0,1); cV=1-math.clamp((y-svBox.AbsolutePosition.Y)/svBox.AbsoluteSize.Y,0,1); svThumb.Position=UDim2.new(cS,0,1-cV,0); rebuildColor(); sw.BackgroundColor3=cur end
        ...
        local hTrack=new("Frame",{Position=UDim2.new(0,12,0.5,-4),Size=UDim2.new(1,-24,0,7),BackgroundColor3=Color3.new(1,1,1)},hRow); cr(99,hTrack)
        new("UIGradient",{Color=ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.fromHSV(0,1,1)),ColorSequenceKeypoint.new(0.17,Color3.fromHSV(0.17,1,1)),ColorSequenceKeypoint.new(0.33,Color3.fromHSV(0.33,1,1)),ColorSequenceKeypoint.new(0.5,Color3.fromHSV(0.5,1,1)),ColorSequenceKeypoint.new(0.67,Color3.fromHSV(0.67,1,1)),ColorSequenceKeypoint.new(0.83,Color3.fromHSV(0.83,1,1)),ColorSequenceKeypoint.new(1,Color3.fromHSV(1,1,1))}},hTrack)
        local hThumb=new("Frame",{AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(cH,0,0.5,0),Size=UDim2.new(0,14,0,14),BackgroundColor3=Color3.new(1,1,1),ZIndex=2},hTrack); cr(99,hThumb); st(T.BD,1,hThumb)
        local function applyH(x) cH=math.clamp((x-hTrack.AbsolutePosition.X)/hTrack.AbsoluteSize.X,0,1); hThumb.Position=UDim2.new(cH,0,0.5,0); svBox.BackgroundColor3=Color3.fromHSV(cH,1,1); rebuildColor(); sw.BackgroundColor3=cur end
        regSlide("hue_"..flg,applyH,function(x,_) applyH(x) end,hTrack)
        local hexRow=new("Frame",{Size=UDim2.new(1,0,0,30),BackgroundTransparency=1,LayoutOrder=nl()},body)
        local hexBg=new("Frame",{Position=UDim2.new(0,12,0.5,-11),Size=UDim2.new(1,-24,0,22),BackgroundColor3=T.B2},hexRow); cr(4,hexBg); local hexSt=st(T.BD,1,hexBg)
        tl({Position=UDim2.new(0,6,0,0),Size=UDim2.new(0,18,1,0),BackgroundTransparency=1,Text="#",TextColor3=T.MT,Font=Enum.Font.GothamBold,TextSize=12},hexBg)
        local hexBox=new("TextBox",{Position=UDim2.new(0,18,0,0),Size=UDim2.new(1,-24,1,0),BackgroundTransparency=1,Text=colorToHex(cur),PlaceholderText="FF69B4",PlaceholderColor3=T.MT,TextColor3=T.TX,Font=Enum.Font.GothamBold,TextSize=12,ClearTextOnFocus=false},hexBg)
        hexBox.Focused:Connect(function() tw(hexSt,{Color=T.A}) end)
        hexBox.FocusLost:Connect(function()
            tw(hexSt,{Color=T.BD}); local c=hexToColor(hexBox.Text)
            if c then cur=c; cH,cS,cV=Color3.toHSV(c); sw.BackgroundColor3=c; svBox.BackgroundColor3=Color3.fromHSV(cH,1,1); svThumb.Position=UDim2.new(cS,0,1-cV,0); hThumb.Position=UDim2.new(cH,0,0.5,0); Flags[flg]=c; if cfg.Callback then cfg.Callback(c) end; saveConfig() end
            hexBox.Text=colorToHex(cur)
        end)
        local origRebuild=rebuildColor; rebuildColor=function() origRebuild(); hexBox.Text=colorToHex(cur) end
    end

    function api:AddLabel(cfg)
        local lbl=tl({Size=UDim2.new(1,0,0,22),BackgroundTransparency=1,Text=cfg.Name or "",TextColor3=T.MT,Font=Enum.Font.FredokaOne,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=nl()},body)
        pad(0,0,12,0,lbl)
    end
    ...
    function api:AddKeybind(cfg)
        local flg=cfg.Flag or cfg.Name
        local cur=Flags[flg] or cfg.Default or Enum.KeyCode.RightShift
        Flags[flg]=cur; local listening=false
        table.insert(searchItems,{label=cfg.Name,keywords=cfg.Name..(cfg.Flag or ""),tab=tabName,switch=tabSwitch})
        local row=new("Frame",{Size=UDim2.new(1,0,0,36),BackgroundTransparency=1,LayoutOrder=nl()},body)
        local hbg=new("Frame",{Size=UDim2.new(1,0,1,0),BackgroundColor3=T.B4,BackgroundTransparency=1},row); cr(4,hbg)
        ...
        local badge=new("TextButton",{AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-10,0.5,0),Size=UDim2.new(0,88,0,22),BackgroundColor3=T.B2,Text=cur.Name,TextColor3=T.A,Font=Enum.Font.GothamBold,TextSize=11},row)
        cr(4,badge); local bSt=st(T.BD,1,badge)
        table.insert(AL,function(c) if not listening then badge.TextColor3=c end end)
        badge.MouseEnter:Connect(function() tw(hbg,{BackgroundTransparency=0.82},0.1) end)
        badge.MouseLeave:Connect(function() tw(hbg,{BackgroundTransparency=1},0.1) end)
        badge.MouseButton1Click:Connect(function()
            if listening then return end
            listening=true; listeningForKey=true
            badge.Text="Press key..."; badge.TextColor3=T.MT
            tw(badge,{BackgroundColor3=T.B4}); tw(bSt,{Color=T.A})
            local conn
            conn=UIS.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.Keyboard then
                    listening=false; listeningForKey=false
                    cur=i.KeyCode; Flags[flg]=cur
...
for i = 1, 30 do
    ...
    local f = new("Frame",{Size=UDim2.new(0,sz,0,sz),BackgroundColor3=Color3.fromRGB(255,182,217),BackgroundTransparency=0.2,ZIndex=6},snowCont)
    cr(99,f)
    flakes[i] = {ui=f, x=math.random(0,100)/100, y=math.random(-10,110)/100, spd=math.random(4,10)/100, dx=math.random(-15,15)/10000}
...
if SNOW_ENABLED then startSnow() end

-- ── WATERMARK ─────────────────────────────────────────────
local wmInitVisible = (Flags["wm"] ~= nil) and Flags["wm"] or WM_SHOW
local wmFrame = new("Frame",{AnchorPoint=Vector2.new(0,1),Position=UDim2.new(0,14,1,-14),Size=UDim2.new(0,0,0,36),AutomaticSize=Enum.AutomaticSize.X,BackgroundColor3=T.B1,BackgroundTransparency=0.08,ZIndex=100,Visible=wmInitVisible},SG)
...
tl({Size=UDim2.new(0,0,0,18),AutomaticSize=Enum.AutomaticSize.X,BackgroundTransparency=1,Text=SCRIPT_NAME,TextColor3=T.TX,Font=Enum.Font.GothamBold,TextSize=14,LayoutOrder=2,ZIndex=101},wmFrame)
